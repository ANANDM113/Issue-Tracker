...
//get element where search issues will be shown
...
searchIssueForm.addEventListener('submit',function(e){
    ...
    searchIssues.map((e1) => {
        if(e1.title ==  titleValue  ||  e1.description  ==  descriptionValue){
            if(!searchedIssues.includes(e1)){
                searchedIssues.push(e1);
            }
        }
    });
        ...
        Div.innerHTML   =   `
            <div class = "card">
                <div class = "card-body">
                    ...
                    <h6 class = "card-subtitle">Description: ${issue.description}</h6>
                </div>
            </div>
        `;
